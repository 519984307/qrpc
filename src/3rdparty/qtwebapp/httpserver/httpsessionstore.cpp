    ...
    :QObject(parent)
{
    ...
    XDebug2("HttpSessionStore: Sessions expire after %i milliseconds",expirationTime);
}
...
QByteArray HttpSessionStore::getSessionId(HttpRequest& request, HttpResponse& response)
{
    ...
    if (!sessionId.isEmpty())
    {
        ...
        {
            XDebug2("HttpSessionStore: received invalid session cookie with ID %s",sessionId.data());
            sessionId.clear();
        }
    }
    mutex.unlock();
...
HttpSession HttpSessionStore::getSession(HttpRequest& request, HttpResponse& response, bool allowCreate)
{
    ...
    if (allowCreate)
    {
        ...
        XDebug2("HttpSessionStore: create new session with ID %s",session.getId().data());
...
void HttpSessionStore::sessionTimerEvent()
{
    ...
    QHash<QByteArray,HttpSession>::iterator i = sessions.begin();
    while (i != sessions.end())
    {
        ...
        {
            XDebug2("HttpSessionStore: session %s expired",session.getId().data());
            sessions.erase(prev);
        }
    }
    mutex.unlock();
