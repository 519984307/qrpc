    ...
    : QObject()
{
    ...
    thread = new QThread();
    thread->start();
    XDebug2("HttpConnectionHandler (%p): thread started", static_cast<void*>(this));
    moveToThread(thread);
    ...
    XDebug2("HttpConnectionHandler (%p): constructed", static_cast<void*>(this));
}
...
void HttpConnectionHandler::thread_done()
{
    ...
    socket->close();
    delete socket;
    XDebug2("HttpConnectionHandler (%p): thread stopped", static_cast<void*>(this));
}
...
HttpConnectionHandler::~HttpConnectionHandler()
{
    ...
    thread->wait();
    thread->deleteLater();
    XDebug2("HttpConnectionHandler (%p): destroyed", static_cast<void*>(this));
}
...
void HttpConnectionHandler::createSocket()
{
    // If SSL is supported and configured, then create an instance of QSslSocket
        ...
        if (sslConfiguration)
        {
            ...
            socket=sslSocket;
            XDebug2("HttpConnectionHandler (%p): SSL is enabled", static_cast<void*>(this));
            return;
        }
#endif
...
void HttpConnectionHandler::handleConnection(const stefanfrings::tSocketDescriptor socketDescriptor)
{
    XDebug2("HttpConnectionHandler (%p): handle new connection", static_cast<void*>(this));
    busy = true;
    ...
    if (!socket->setSocketDescriptor(socketDescriptor))
    {
        ...
        if (sslConfiguration)
        {
            XDebug2("HttpConnectionHandler (%p): Starting encryption", static_cast<void*>(this));
...
{
    XDebug2("HttpConnectionHandler (%p): read timeout occured",static_cast<void*>(this));
...
{
    XDebug2("HttpConnectionHandler (%p): disconnected", static_cast<void*>(this));
    socket->close();
    readTimer.stop();
    busy = false;
}
...
void HttpConnectionHandler::read()
{
    ...
    {
        #ifdef SUPERVERBOSE
            XDebug2("HttpConnectionHandler (%p): read input",static_cast<void*>(this));
    #endif
        ...
        {
            readTimer.stop();
            XDebug2("HttpConnectionHandler (%p): received request",static_cast<void*>(this));
            ...
            XDebug2("HttpConnectionHandler (%p): finished request",static_cast<void*>(this));
