...
HttpConnectionHandlerPool::~HttpConnectionHandlerPool()
{
    ...
    XDebug2("HttpConnectionHandlerPool (%p): destroyed", this);
}
    ...
    foreach(HttpConnectionHandler* handler, pool)
    {
        if (!handler->isBusy())
        {
            if (++idleCounter > maxIdleHandlers)
            {
                ...
                XDebug3("HttpConnectionHandlerPool: Removed connection handler (%p), pool size is now %i",handler,pool.size());
...
void HttpConnectionHandlerPool::loadSslConfig()
{
    ...
    QString sslKeyFileName=settings->value(QStringLiteral("sslKeyFile"),"").toString();
    QString sslCertFileName=settings->value(QStringLiteral("sslCertFile"),"").toString();
