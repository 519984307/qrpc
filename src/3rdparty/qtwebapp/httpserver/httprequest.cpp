...
HttpRequest::HttpRequest(const QSettings* settings)
{
    ...
    currentSize=0;
    expectedBodySize=0;
    maxSize=settings->value(QByteArrayLiteral("maxRequestSize"), QByteArrayLiteral("16000")).toInt();
    maxMultiPartSize=settings->value(QByteArrayLiteral("maxMultiPartSize"), QByteArrayLiteral("1000000")).toInt();
    tempFile=nullptr;
}
...
{
    #ifdef SUPERVERBOSE
        XDebug2("HttpRequest: read request");
#endif
    ...
    if (!lineBuffer.contains(QByteArrayLiteral("\r\n")))
    {
        #ifdef SUPERVERBOSE
            XDebug2("HttpRequest: collecting more parts until line break");
    #endif
        return;
    }
    ...
    if (!newData.isEmpty())
    {
        //XDebug2("HttpRequest: from %s: %s",qPrintable(socket->peerAddress().toString()),newData.data());
        QList<QByteArray> list=newData.split(' ');
        if (list.count()!=3 || !list.at(2).contains(QByteArrayLiteral("HTTP")))
        {
...
void HttpRequest::readHeader(QTcpSocket* socket)
{
    ...
    if (colon>0)
    {
        ...
        #ifdef SUPERVERBOSE
            XDebug2("HttpRequest: received header %s: %s",currentHeader.data(),value.data());
    #endif
    }
    else if (!newData.isEmpty())
    {
        ...
        #ifdef SUPERVERBOSE
            XDebug2("HttpRequest: read additional line of header");
    #endif
    ...
    else
    {
        ...
        #ifdef SUPERVERBOSE
            XDebug2("HttpRequest: headers completed");
    #endif
        // Empty line received, that means all headers have been received
        // Check for multipart/form-data
        QByteArray contentType=headers.value(QByteArrayLiteral("content-type"));
        if (contentType.startsWith(QByteArrayLiteral("multipart/form-data")))
        {
            int posi=contentType.indexOf(QByteArrayLiteral("boundary="));
            if (posi>=0) {
        ...
        QByteArray contentLength=headers.value(QByteArrayLiteral("content-length"));
        ...
        {
            #ifdef SUPERVERBOSE
                XDebug2("HttpRequest: expect no body");
        #endif
            status=complete;
        }
        ...
        else if (!boundary.isEmpty() && expectedBodySize>maxMultiPartSize)
        {
            ...
            #ifdef SUPERVERBOSE
                XDebug2("HttpRequest: expect %i bytes body",expectedBodySize);
        #endif
            status=waitForBody;
        }
    }
...
void HttpRequest::readBody(QTcpSocket* socket)
{
    ...
    if (boundary.isEmpty())
    {
        ...
        #ifdef SUPERVERBOSE
            XDebug2("HttpRequest: receive body");
    #endif
    ...
    else
    {
        ...
        #ifdef SUPERVERBOSE
            XDebug2("HttpRequest: receiving multipart body");
    #endif
        ...
        {
        #ifdef SUPERVERBOSE
            XDebug2("HttpRequest: received whole multipart body");
    #endif
            tempFile->flush();
            if (tempFile->error())
            {
...
{
    #ifdef SUPERVERBOSE
        XDebug2("HttpRequest: extract and decode request parameters");
#endif
    // Get URL parameters
    ...
    QByteArray contentType=headers.value(QByteArrayLiteral("content-type"));
    if (!bodyData.isEmpty() && (contentType.isEmpty() || contentType.startsWith(QByteArrayLiteral("application/x-www-form-urlencoded"))))
    {
    ...
    foreach (QByteArray part, list)
    {
        ...
        {
            // Name without value
            parameters.insert(urlDecode(part), QByteArrayLiteral(""));
        }
    }
}
...
{
    #ifdef SUPERVERBOSE
        XDebug2("HttpRequest: extract cookies");
#endif
    foreach(QByteArray cookieStr, headers.values(QByteArrayLiteral("cookie")))
    {
        QList<QByteArray> list=HttpCookie::splitCSV(cookieStr);
        foreach(QByteArray part, list)
        {
            #ifdef SUPERVERBOSE
                XDebug2("HttpRequest: found cookie %s",part.data());
    ...
    headers.remove(QByteArrayLiteral("cookie"));
}
...
{
    XDebug1("HttpRequest: parsing multipart temp file");
    tempFile->seek(0);
    bool finished=false;
    while (!tempFile->atEnd() && !finished && !tempFile->error())
    {
        #ifdef SUPERVERBOSE
            XDebug2("HttpRequest: reading multpart headers");
    #endif
        ...
        while (!tempFile->atEnd() && !finished && !tempFile->error())
        {
            ...
            if (line.startsWith("Content-Disposition:"))
            {
                if (line.contains("form-data"))
                {
                    ...
                    #ifdef SUPERVERBOSE
                        XDebug2("HttpRequest: multipart field=%s, filename=%s",fieldName.data(),fileName.data());
                #endif
                }
                else
                {
                    XDebug2("HttpRequest: ignoring unsupported content part %s",line.data());
                }
            }
        ...
        #ifdef SUPERVERBOSE
            XDebug2("HttpRequest: reading multpart data");
    #endif
        ...
        while (!tempFile->atEnd() && !finished && !tempFile->error())
        {
            ...
            if (line.startsWith("--"+boundary))
            {
                ...
                if (fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    //XDebug2("HttpRequest: set parameter %s=%s",fieldName.data(),fieldValue.data());
                }
                else if (!fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    {
                        #ifdef SUPERVERBOSE
                            XDebug2("HttpRequest: finishing writing to uploaded file");
                    #endif
                        uploadedFile->resize(uploadedFile->size()-2);
                        uploadedFile->flush();
                        uploadedFile->seek(0);
                        parameters.insert(fieldName,fileName);
                        //XDebug2("HttpRequest: set parameter %s=%s",fieldName.data(),fileName.data());
                        uploadedFiles.insert(fieldName,uploadedFile);
                        XDebug2("HttpRequest: uploaded file size is %lli",uploadedFile->size());
                    }
                    else
                    {
    ...
    #ifdef SUPERVERBOSE
        XDebug2("HttpRequest: finished parsing multipart temp file");
#endif
}
