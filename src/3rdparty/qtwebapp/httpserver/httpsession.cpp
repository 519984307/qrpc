...
#ifdef SUPERVERBOSE
        XDebug2("HttpSession: (constructor) new session %s with refCount=1",dataPtr->id.constData());
#endif
    }
    else
    {
...
HttpSession::HttpSession(const HttpSession& other)
{
    ...
    if (dataPtr)
    {
        ...
        dataPtr->refCount++;
#ifdef SUPERVERBOSE
        XDebug2("HttpSession: (constructor) copy session %s refCount=%i",dataPtr->id.constData(),dataPtr->refCount);
#endif
        dataPtr->lock.unlock();
    }
}
...
HttpSession& HttpSession::operator= (const HttpSession& other)
{
    ...
    if (dataPtr)
    {
        ...
        dataPtr->refCount++;
#ifdef SUPERVERBOSE
        XDebug2("HttpSession: (operator=) session %s refCount=%i",dataPtr->id.constData(),dataPtr->refCount);
#endif
        dataPtr->lastAccess=QDateTime::currentMSecsSinceEpoch();
        dataPtr->lock.unlock();
    }
...
#ifdef SUPERVERBOSE
        XDebug2("HttpSession: (operator=) session %s refCount=%i",oldPtr->id.constData(),oldPtr->refCount);
#endif
        oldPtr->lock.unlock();
        if (refCount==0)
        {
            XDebug1("HttpSession: deleting old data");
            delete oldPtr;
        }
    }
    return *this;
...
#ifdef SUPERVERBOSE
        XDebug2("HttpSession: (destructor) session %s refCount=%i",dataPtr->id.constData(),dataPtr->refCount);
#endif
        dataPtr->lock.unlock();
        if (refCount==0)
        {
            XDebug1("HttpSession: deleting data");
            delete dataPtr;
        }
    }
}
...
QHash<QByteArray,QVariant> HttpSession::getAll() const
{
