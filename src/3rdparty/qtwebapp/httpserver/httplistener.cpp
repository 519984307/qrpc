...
{
    close();
    XDebug1("HttpListener: destroyed");
}
...
void HttpListener::incomingConnection(tSocketDescriptor socketDescriptor) {
#ifdef SUPERVERBOSE
    XDebug1("HttpListener: New connection");
#endif
    ...
    if (freeHandler)
    {
        ...
        QMetaObject::invokeMethod(freeHandler, "handleConnection", Q_ARG(tSocketDescriptor, socketDescriptor));
    }
    else
    {
        // Reject the connection
        XDebug1("HttpListener: Too many incoming connections");
