    ...
    :HttpRequestHandler(parent)
{
    ...
    XDebug4("StaticFileController: docroot=%s, encoding=%s, maxAge=%i",qPrintable(docroot),qPrintable(encoding),maxAge);
    maxCachedFileSize=settings->value("maxCachedFileSize","65536").toInt();
    cache.setMaxCost(settings->value("cacheSize","1000000").toInt());
    cacheTimeout=settings->value("cacheTime","60000").toInt();
    XDebug3("StaticFileController: cache timeout=%i, size=%i",cacheTimeout,cache.maxCost());
}
...
void StaticFileController::service(HttpRequest &request, HttpResponse &response)
{
    ...
    if (entry && (cacheTimeout==0 || entry->created>now-cacheTimeout))
    {
        ...
        mutex.unlock();
        XDebug2("StaticFileController: Cache hit for %s",path.data());
    ...
    else
    {
        ...
        XDebug2("StaticFileController: Cache miss for %s",path.data());
        ...
        XDebug2("StaticFileController: Open file %s",qPrintable(file.fileName()));
...
void StaticFileController::setContentType(const QString fileName, HttpResponse &response) const
{
    ...
    {
        XDebug2("StaticFileController: unknown MIME type for filename '%s'", qPrintable(fileName));
    }
}
